...
public class TransLoss extends BankTrans {
	...
	public int doTrans() throws Exception {
		...
		if (!acct.acctStatusVerify()) {
			setTrans_result("账户已销户或已挂失");
			return -1;
		}
		System.out.println("输入挂失原因:");
		String cause = scn.next();
		// 挂失
		acct.setAcct_status(3);
		if (acct.regLoss() < 0) {
			setTrans_result("挂失失败!");
			return -1;
		}
		setTrans_result("挂失成功");
		System.out.println("挂失原因:" + cause);
		return 0;
	}
